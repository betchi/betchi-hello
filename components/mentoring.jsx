...
export class MentoringPage extends React.Component {
	...
	onScroll(e) {
		...
		if (bottom <= 60) {
			window.removeEventListener('scroll', this.onScroll);
			this.loadContents(this.props.params.id, this.state.page + 1);
		}
	}

	loadContents(id = 1, page = 1) {
		let xhr = new XMLHttpRequest();
		xhr.open('GET', '/thxmessage.json?p=' + page);
		xhr.onload = () => {
			if (xhr.status !== 200) {
				this.setState({
					snack: {
	...
	render() {
		...
		return (
			<section style={styles.root}>
				...
				<MentoringCoverSwipe
					covers={['/cover.jpg','/cover.jpg','/cover.jpg']}
					title={'タイトル'}
					duration={'1時間'}
					price={'いいね値段'}
				/>
